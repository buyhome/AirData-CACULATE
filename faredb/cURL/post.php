 ...
 * http post
 */
function httpRequest($url, $post_string, $is_post = TRUE, $connectTimeout = CONNECT_TIMEOUT, $readTimeout = READ_TIMEOUT) {
	$result = "";
	if (function_exists('curl_init')) {
		...
		curl_setopt($ch, CURLOPT_POST, (boolean)$is_post);
 ...
 * http post
 */
function socketPost($url, $post_string, $is_post = TRUE, $connectTimeout = CONNECT_TIMEOUT, $readTimeout = READ_TIMEOUT) {
	...
	/* begin send data */
	$http_method = (boolean)$is_post ? 'POST' : 'GET';
	$in = $http_method . " " . $urlInfo["request"] . " HTTP/1.0\r\n";
	$in .= "Accept: */*\r\n";
	$in .= "User-Agent: rhomobi.com API PHP5 Client 1.0(non-curl)\r\n";
