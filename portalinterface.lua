...
if ngx.var.request_method == "POST" then
		...
		if tonumber(content.DEPARTUREDATE) < tonumber(reqdate) then
			...
			if content.AIRLINE ~= JSON.null then
				...
				if table_is_empty(fareresult) == nil then
					...
					if not keyres then
						...
						if table_is_empty(keyres) == nil then
							...
							-- del fresid
							local dreys, dreyerr = csd:smembers("cac:" .. ckey .. ":freskey")
							if not dreys then
								ngx.say(error004("failed to smembers cac:" .. ckey .. ":freskey", dreyerr));
								return
							else
								for k, v in pairs(dreys) do
				...
				if table_is_empty(fareresult) == nil then
					...
					-- do NOT for.
					--[[
					-- get the avhcmd of the CITY_PATH
					local cmds, cmderr = red:smembers("ORGDST:" .. city .. ":CMD")
					if not cmds then
						ngx.say("failed to get the [ORGDST:" .. city .. "'s CMD]", cmderr);
						return
					else
						for key, val in pairs(cmds) do
							local sinterok, sintererr = red:sinter("AVHCMD:" .. val, )
							if sinterok ~= nil then
								table.insert(reqs, cmds)
							end
						end
					end
					--]]
					-- first version do foreach fid
					for k, v in pairs(fareresult) do
						local isert = ture;
						local res, err = red:get("fare:" .. v .. ":AVHCMD")
						if not res then
							ngx.say(error003("failed to get the [fare:" .. v .. "'s AVHCMD]", err));
							return
						end
						-- ngx.say(res);
						local reqlen = table.getn(reqs);
						local avhmuli = "/data-avh/" .. string.lower(res) .. content.DEPARTUREDATE .. "/";
						-- ngx.say(ngx.md5(avhmuli))
						if reqlen == 0 then
							table.insert(reqs, { avhmuli })
						else
							local reqi = 1;
							while reqi <= reqlen do
								-- ngx.say(reqs[reqi][1])
								if reqs[reqi][1] == avhmuli then
									isert = false;
								end
								reqi = reqi + 1;
							end
						...
						if not resnum then
							...
							-- fare:$fid:BASE_AIRLINE
							local ba, baerr = red:get("fare:" .. v .. ":BASE_AIRLINE")
							if not ba then
								ngx.say(error003("failed to get the fare:" .. ba .. "'s BASE_AIRLINE", baerr));
								return
							else
								table.insert(tmpfres, { string.upper(ba) })
								-- CITY_PATH==airport_path
								local res, err = red:get("fare:" .. v .. ":CITY_PATH")
								if not res then
									ngx.say(error003("failed to get the fare:" .. v .. "'s CITY_PATH", err));
									return
								else
									-- ngx.say(res);
									table.insert(tmpfres, { string.upper(res) })
							...
							for idx, value in ipairs(avcontent.avItems) do
								...
								if not dpres then
									...
									for key, value1 in ipairs(value.segments) do
										if value1.carrier == JSON.null then
											-- table.insert(tmpavhres, { string.upper(content.AIRLINE) })
											table.insert(tmpavhres, { string.sub(string.upper(value1.airline), 1, 2) })
										else
											table.insert(tmpavhres, { string.sub(string.upper(value1.carrier), 1, 2) })
										end
										if airport2 == value1.orgcity then
									...
									-- ngx.say(string.len(airportpath));
									-- ngx.say(reqs[i][1]);
									local baline = string.sub(reqs[i][1], 19, 20);
									table.insert(tmpavhres, { string.upper(baline) })
									table.insert(tmpavhres, { string.upper(airportpath) })
									-- ngx.say(tmpavhres);
									-- ngx.say(itemcount);
									-- avh result caculate for avhkey.
	...
	if not rok then
		ngx.say(error003("failed to set keepalive with Faredata server: ", rerr));
		return
	end
	local cok, cerr = csd:set_keepalive(0, 512)
	if not cok then
		ngx.say(error004("failed to set keepalive with Caculate server: ", cerr));
		return
	end
